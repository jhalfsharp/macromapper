...
function UTF8ToString(ptr, maxBytesToRead) {
	...
	while (!(idx >= endIdx)) {
		...
		if ((u0 & 0xf0) == 0xe0) {
			...
			if ((u0 & 0xf8) != 0xf0)
				console.warn(
					'Invalid UTF-8 leading byte 0x' + u0.toString(16) + ' encountered when deserializing a UTF-8 string on the asm.js/wasm heap to a JS string!'
				);
...
function resize(canvas, on_resize) {
	...
	if (canvas.width != displayWidth || canvas.height != displayHeight) {
		...
		if (on_resize != undefined) on_resize(Math.floor(displayWidth), Math.floor(displayHeight));
	}
}
...
function init_plugins(plugins) {
	...
	for (var i = 0; i < plugins.length; i++) {
		...
		if (plugins[i].name == undefined || plugins[i].name == null || plugins[i].version == undefined || plugins[i].version == null) {
